...
9. **Iter 9 (planned)** — *Saw* the Iter 8 math more clearly: vLLM's drain rate (`max-num-seqs / per-call decode time = 32 / 4 s ≈ 8 calls/s`) is ~ 3× below agent demand (`10 RPS × ~ 2.5 calls/question ≈ 25 calls/s`). Queue *has* to grow under that imbalance — it's arithmetic, not a config bug. Two ways to close the gap: raise the cap (numerator) or reduce per-call decode time (denominator). Raising the cap risks re-triggering Iter 7's decode-amplification cascade; reducing per-call decode time has no such tail risk. *Hypothesized* bumping n-gram speculation aggressiveness from `num_speculative_tokens=4, prompt_lookup_max=4` (Iter 6 acceptance rate = 63.6 %) to `6 / 8` would push per-call decode from ~ 4 s toward ~ 2.5 s, lifting vLLM's effective drain at the same cap=32 to ~ 12-13 calls/s — much closer to balance with demand, while the cap still prevents in-flight runaway. Speculation has zero quality risk by construction (verifier rejects bad guesses) and is a single config change with no code. *Changed* `scripts/start_vllm.sh` speculation config from `4 / 4` to `6 / 8`; cap=32, workers=4, agent code all unchanged so the iteration isolates the speculation effect. *Result* (`results/load_test_iter9_speculation_tuned.json`): **TODO after re-run**.

10. **Iter 10 (conditional, planned)** — Branch on Iter 9 outcome: (a) if SLO hit → done, write up the verdict; (b) if P95 lands 5-15 s and speculation acceptance stayed above ~ 40 % → relax `--max-num-seqs` from 32 to 48 for additional drain headroom now that per-call decode is cheaper; (c) if P95 still > 15 s *or* speculation acceptance collapsed → per-question **schema pruning** in `agent/schema.py` becomes mandatory (render only top-k tables by question-keyword overlap + FK neighbors, drops worst-case 5 K-token schemas to ~ 1 K tokens, addresses the workload heterogeneity directly).
