...
4. **Iter 4** — Reverted to launch-time worker scaling so each process gets its own httpx connection pool, and removed the in-process lifespan handler from `agent/server.py`. *Hypothesized* 5 workers × 40 = 200 slots (≈ 25 % more than Iter 1's 160) would land at ~78 % KV peak by linear extrapolation from Iter 1's 62 % — a safe step past Iter 1 that avoids Iter 3's overshoot, with 5 × 100 = 500 httpx connections so the connection-pool failure mode cannot recur. *Changed* the launch command to `uvicorn agent.server:app --workers 5`. *Result* (`results/load_test_iter4_workers5.json`): **TODO after re-run**.
