...
10. **Iter 10** — *Hypothesized* the recurring blocker since Iter 7 has one root cause: 12.9 % of the load pool comes from two DBs (`debit_card_specializing` + `european_football_2`) with ~ 5 K-token schemas, dominating KV slot occupancy and per-call wall-clock. The only structural fix is to **stop rendering 5 K-token schemas** in the first place — render per-question only the tables a question is likely to need, dropping the worst-case schema to ~ 1 K tokens. *Changed* `agent/schema.py` to accept a `question` argument; new logic scores each table by keyword overlap between the question and the table's name + column names, keeps top-k (k=6), then expands by one-hop FK neighbors (both directions) so join paths are preserved. Backward-compatible fallback: empty question or zero-overlap scoring → render all tables (old behavior). `agent/graph.py:_attach_schema` updated to pass `state.question`. Also reverted speculation from `6 / 8` back to `4 / 4` since Iter 9 showed `6 / 8` was actively harmful — keeps Iter 6's known-good acceptance rate (63.6 %) as a stable baseline so the iteration isolates the schema-pruning effect. *Result* (`results/load_test_iter10_schemapruning.json`): **TODO after re-run**.
